...
- Replace: schedule 주기가 돌아왔을 때, 앞의 주기에 만들었던 Pod가 종료되지 않은 상태라면 새로운 Job을 실행하여 Pod를 생성하는 것이 아니라 기존의 Job을 사용하여 새로운 Pod를 생성합니다. 즉 다음 주기가 됐을 때 새로운 Job의 생성은 없고 새로운 Pod의 생성과 이전 Job의 연결만 발생합니다. 그리고 Pod의 수행이 완료되면 새로운 Job을 실행하여 다음 Pod를 생성하게 됩니다.
